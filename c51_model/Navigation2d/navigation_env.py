...
class NavigationEnvDefault(gym.Env, EzPickle):
    ...
    def __init__(self, task_args, max_obs_range=3,  max_speed=2, initial_speed=2, **kwargs):
        ...
        self.obstacles = self.obstacle_cls(self.world, max_speed, task_args)
...
class NavigationEnvAcc(NavigationEnvDefault):
    ...
    def step(self, action: np.iterable):
        action = np.asarray(action, dtype=np.float64)
        noise = self.action_space.sample()
...
class DeployEnv(NavigationEnvAcc):
    def __init__(self, obstacles_args, max_obs_range=3, max_speed=5, initial_speed=2, noise_weight=0.1, **kwargs):
        super().__init__(obstacles_args, max_obs_range, max_speed, initial_speed, )
        self.noise_weight = noise_weight
        self.action_weight = 1 - noise_weight


    def step(self, action: np.iterable):
        action = np.asarray(action, dtype=np.float64)
        noise = self.action_space.sample()
        action = self.action_weight * action + self.noise_weight * noise
